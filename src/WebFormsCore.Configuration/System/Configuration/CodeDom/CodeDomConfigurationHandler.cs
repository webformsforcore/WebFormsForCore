...
namespace EstrellasDeEsperanza.WebFormsCore.CodeDom.Compiler
{
	...
	public class CodeDomCompilationConfiguration
	{
		...
		internal CodeDomCompilationConfiguration()
		{
			...
			W.CompilerInfo compilerInfo;
			S.CompilerParameters compilerParameters;
			String typeName;

			// C#
			compilerParameters = new S.CompilerParameters();
			compilerParameters.WarningLevel = 4;
			typeName = "EstrellasDeEsperanza.WebFormsCore.CodeDom.Compiler.CSharpCodeProvider, " + AssemblyRef.WebFormsCoreWeb;
			...
			compilerParameters = new S.CompilerParameters();
			compilerParameters.WarningLevel = 4;
			typeName = "EstrellasDeEsperanza.WebFormsCore.CodeDom.Compiler.VBCodeProvider, " + AssemblyRef.WebFormsCoreWeb;
			compilerInfo = new W.CompilerInfo(compilerParameters, typeName);
		...
		private void AddCompilerInfo(W.CompilerInfo compilerInfo)
		{
		...
		private void RemoveUnmapped()
		{
			...
			{
				((W.CompilerInfo)_allCompilerInfo[i])._mapped = false;
			}

			// Re-mark only the ones that still have a mapping
			foreach (W.CompilerInfo destinationCompilerInfo in _compilerLanguages.Values)
			{
				destinationCompilerInfo._mapped = true;
			}
			foreach (W.CompilerInfo destinationCompilerInfo in _compilerExtensions.Values)
			{
				destinationCompilerInfo._mapped = true;
			}
			...
			{
				if (!((W.CompilerInfo)_allCompilerInfo[i])._mapped)
				{
		...
		internal class SectionHandler
		{
			...
			private static void ProcessCompilersElement(CodeDomCompilationConfiguration result, XmlNode node)
			{
				...
				foreach (XmlNode child in node.ChildNodes)
				{
					...
					S.CompilerParameters compilerParams = new S.CompilerParameters();
					...
					W.CompilerInfo compilerInfo = null;
					if (compilerTypeName != null)
					{
						compilerInfo = new W.CompilerInfo(compilerParams, compilerTypeName);
					}
					else
					{
		...
		private W.CompilerInfo FindExistingCompilerInfo(string[] languageList, string[] extensionList)
		{
			W.CompilerInfo compilerInfo = null;

			foreach (W.CompilerInfo ci in _allCompilerInfo)
			{
