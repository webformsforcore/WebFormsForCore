...
internal static class BrowserDetectionUtility
{
	...
	public static bool IsIE55OrHigher(NameValueCollection browserCapabilities)
	{
		...
		if (!int.TryParse(browserCapabilities[MajorVersionKey], out var result) || !double.TryParse(browserCapabilities[MinorVersionKey], out var result2))
		{
			return false;
		}
	...
	public static bool IsChrome(string userAgent)
	{
		if (userAgent != null)
		{
			return userAgent.IndexOf(ChromeUserAgent, StringComparison.OrdinalIgnoreCase) >= 0;
		}
		return false;
	}
