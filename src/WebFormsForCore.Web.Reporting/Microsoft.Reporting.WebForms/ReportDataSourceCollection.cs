...
public sealed class ReportDataSourceCollection : SyncList<ReportDataSource>, ISerializable
{
		...
		: this(new object())
	{
		...
		for (int i = 0; i < @int; i++)
		{
			...
			{
				Name = info.GetString("Name" + text),
				DataSourceId = info.GetString("ID" + text)
			});
		}
	}
	...
	void ISerializable.GetObjectData(SerializationInfo info, StreamingContext context)
	{
		...
		for (int i = 0; i < base.Count; i++)
		{
			...
			info.AddValue("Name" + text, reportDataSource.Name);
			info.AddValue("ID" + text, reportDataSource.DataSourceId);
		}
	}
