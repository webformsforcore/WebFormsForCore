...
public sealed class ServerReport : Report, ISerializable
{
	...
	public IReportServerCredentials ReportServerCredentials
	{
		get
		{
			return m_serverCredentials;
		}
		set
		{
			lock (m_syncObject)
			{
				if (value != m_serverCredentials)
				{
					if (base.IsDrillthroughReport)
					{
						throw new InvalidOperationException();
	...
	internal string ReportUrlNoScript => null;
		...
		: this()
	{
		...
		foreach (Cookie cookie in original.Cookies)
		{
			Cookies.Add(cookie);
		}
		ReportServerCredentials = original.ReportServerCredentials;
	}
	...
	internal string SerializeToUrlQuery()
	{
		lock (m_syncObject)
		{
			return string.Format(CultureInfo.InvariantCulture, "{0}={1}", "ReportSession", m_executionID);
		}
	}
	...
	internal void LoadFromUrlQuery(NameValueCollection requestParameters, bool fullReportLoad)
	{
		lock (m_syncObject)
		{
			string text = requestParameters["ReportSession"];
			if (!string.IsNullOrEmpty(text))
			{
				SetExecutionId(text, fullReportLoad);
			}
		}
	}
	...
	internal void InternalRender(bool isAbortable, string format, string deviceInfo, NameValueCollection urlAccessParameters, Stream reportStream, out string mimeType, out string fileNameExtension)
	{
		lock (m_syncObject)
		{
			...
			if (!string.IsNullOrEmpty(deviceInfo))
			{
				...
				StringReader input = new StringReader(deviceInfo);
	...
	{
		if (!m_executionInfo.HasSnapshot || m_executionInfo.NeedsProcessing || PageCountMode != PageCountMode.Actual)
		{
	...
	internal bool IsPrintCabSupported(ClientArchitecture arch)
	{
		lock (m_syncObject)
		{
			return Service.IsPrintCabSupported(arch);
		}
	}
	...
	internal string GetPrintControlClsid(ClientArchitecture arch)
	{
		lock (m_syncObject)
		{
			return Service.GetPrintControlClsid(arch);
		}
	}
	...
	internal void WritePrintCab(ClientArchitecture arch, Stream stream)
	{
		lock (m_syncObject)
		{
			Service.WritePrintCab(arch, stream);
		}
	}

	internal string CreateStyleSheetUrl(string styleSheetName)
	{
		return ReportServerStyleSheetOperation.CreateUrl(styleSheetName, GetServerVersion(), isImage: false);
	}

	internal string CreateStyleSheetImageUrl(string styleSheetImageName)
	{
		return ReportServerStyleSheetOperation.CreateUrl(styleSheetImageName, GetServerVersion(), isImage: true);
	}

	internal byte[] GetStyleSheet(string styleSheetName, bool isImage, out string mimeType)
	{
		return Service.GetStyleSheet(styleSheetName, isImage, out mimeType);
	}
	...
	private int GetIndexForParameter(string parameterName)
	{
		...
		{
			ReportParameterInfo reportParameterInfo = m_executionInfo.Parameters[i];
			if (string.Compare(reportParameterInfo.Name, parameterName, StringComparison.OrdinalIgnoreCase) == 0)
			{
				num = i;
				break;
			}
