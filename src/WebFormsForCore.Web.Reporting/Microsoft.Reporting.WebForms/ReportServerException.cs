...
public class ReportServerException : ReportViewerException
{
	...
	{
		if (e is ReportServerException result)
		{
			return result;
		}
		if (e is SoapException ex)
		{
			XmlNode nestedMoreInformationNode = GetNestedMoreInformationNode(ex.Detail);
			ReportServerException ex2 = FromMoreInformationNode(nestedMoreInformationNode);
			if (ex2 != null)
			{
				return ex2;
			}
	...
	internal static ReportServerException FromMoreInformationNode(XmlNode moreInfoNode)
	{
		...
		XmlNode nestedMoreInformationNode = GetNestedMoreInformationNode(moreInfoNode);
		ReportServerException ex = FromMoreInformationNode(nestedMoreInformationNode);
