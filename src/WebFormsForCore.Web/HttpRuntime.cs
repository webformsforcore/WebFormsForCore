...
#if !WebFormsForCore && !FEATURE_PAL // FEATURE_PAL does not enable IIS-based hosting features
...
#else 
                            throw new NotSupportedException("Prefetch optimization not supported in WebFormsForCore.");
#endif
                        }
                    }
                ...
                try {
                    using (new ApplicationImpersonationContext()) {
                        ...
                        try {
							...
							HttpResponse.RethrowIfResponseEnd(() => context?.Response);
						}
