...
public sealed class BuildManager
{
    ...
    internal static void ResetBuildManager() {
        _theBuildManager = new BuildManager();
        s_topLevelHash = 0;
    ...
    private void LoadLicensesAssemblyIfExists()
    {
        ...
        if (File.Exists(licAssemblyPath))
        {
            ...
            alc.LoadFromAssemblyName(new AssemblyName(LicensesAssemblyName));
        }
    }
    ...
    internal static ICollection<Assembly> LoadCachedPreAppStartAssemblies(string preStartInitListPath)
    {
        try
        {
            ...
            var alc = AssemblyLoadContext.GetLoadContext(Assembly.GetExecutingAssembly());
            return assemblyList.Select(a => alc.LoadFromAssemblyName(new AssemblyName(a)))
                               .Distinct()
                               .ToList();
        }
        catch
    ...
    private Assembly ResolveAssembly(object sender, ResolveEventArgs e)
    {
            ...
            e.RequestingAssembly != null)
        {
            ...
            if (File.Exists(assemblyFile)) {
                var alc = AssemblyLoadContext.GetLoadContext(Assembly.GetExecutingAssembly());
                return alc.LoadFromAssemblyPath(assemblyFile);
            }
        }
#endif
        return null;
