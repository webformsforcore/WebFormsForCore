    ...
    public class AssemblyPatcher
    {
        ...
        static readonly Dictionary<string, string> mappings = new Dictionary<string, string>(
            new KeyValuePair<string, string>[] {
                new KeyValuePair<string, string>("System.Web", "WebFormsForCore.Web"),
                new KeyValuePair<string, string>("System.Configuration", "WebFormsForCore.Configuration"),
                new KeyValuePair<string, string>("System.Web.RegularExpressions", "WebFormsForCore.Web.RegularExpressions"),
                new KeyValuePair<string, string>("System.Web.ApplicationServices", "WebFormsForCore.Web.ApplicationServices"),
                new KeyValuePair<string, string>("System.Web.Services", "WebFormsForCore.Web.Services") });
        ...
        class DestinationAssemblies : KeyedCollection<string, AssemblyDefinition>
        {
            ...
            public bool IsWebFormsForCore(AssemblyDefinition a) => destinationAssemblyNames.Contains(a.Name.Name);

			protected override void InsertItem(int index, AssemblyDefinition item)
			{
				if (IsWebFormsForCore(item)) base.InsertItem(index, item);
			}
			...
			{
				if (IsWebFormsForCore(item)) base.SetItem(index, item);
			}
		}
        ...
        public void CreateForwarderAssemblies(IEnumerable<string> files, string signWithKeyFile)
        {
            ...
            foreach (var ad in assemblies) {
                if (mappings.ContainsKey(ad.Assembly.Name)) {
                    destinations.AddIfWebFormsForCore(new AssemblyNameReference(mappings[ad.Assembly.Name];
            */
            foreach (var ad in assemblies)
            {
